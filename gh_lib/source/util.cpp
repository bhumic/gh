    ...
    namespace process
    {
        ...
        void get_process_info(const std::string exe_name, gh::process::PROCESS_INFO& process_info)
        {
            ...
            if (snapshot_pid != INVALID_HANDLE_VALUE)
                process_info.pid = gh::process::get_process_id(snapshot_pid, exe_name);
            else
                gh::error::print_error(GetLastError());
            ...
            if (process_info.pid)
            {
                ...
                if (snapshot_base != INVALID_HANDLE_VALUE)
                    process_info.base = gh::process::get_process_base(snapshot_base, exe_name);
                else
                    gh::error::print_error(GetLastError());
            ...
            if (process_info.pid)
            {
                ...
                if (snapshot_tid != INVALID_HANDLE_VALUE)
                    process_info.tid = gh::process::get_thread_id(snapshot_tid, process_info.pid);
                else
                    gh::error::print_error(GetLastError());
            ...
            {
                process_info.is64bit = gh::process::is_64bit(process_info.handle);
            }
        }
